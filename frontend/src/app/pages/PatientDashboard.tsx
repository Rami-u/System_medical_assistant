...
import {
  ...
  TrendingUp, TrendingDown, BarChart2, FileText, Eye,
} from "lucide-react";
import {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-5 py-6">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            {/* ── High-Risk DR Alert Banner ──────────────────────────────────── */}
            {data?.risk_level === "high" && (
              <button
                onClick={() => navigate("/dashboard/patient/retinopathy")}
                className="w-full flex items-start gap-3 bg-red-50 border border-red-200 rounded-2xl px-5 py-4 text-left hover:bg-red-100 transition-colors group"
              >
                <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                  <Eye className="w-5 h-5 text-red-600" strokeWidth={1.8} />
                </div>
                <div className="flex-1">
                  <p className="text-red-800 text-sm" style={{ fontWeight: 700 }}>
                    Eye Screening Recommended
                  </p>
                  <p className="text-red-600 text-xs mt-0.5 leading-relaxed">
                    Based on your risk profile, we recommend an AI-powered retinal scan for early detection
                    of diabetic retinopathy. Tap here to start.
                  </p>
                </div>
                <ChevronRight className="w-4 h-4 text-red-400 group-hover:text-red-600 flex-shrink-0 mt-1 transition-colors" />
              </button>
            )}
